...
const createGameBoardPixels = () => {
  ...
  for (let i = 1; i <= SQUARE_OF_GAME_PIXEL_COUNT; ++i) {
    gamePixelDivs = `${gamePixelDivs} <div class="game-pixel" id="pixel${i}"></div>`;
  }
...
const gameBoardPixels = document.getElementsByClassName("game-pixel");
...
const moveSnake = () => {
  ...
  // Update in UI:
  document.getElementById("total-blocks").innerHTML = totalDistanceTravelled;

  if (currentSnakeHeadPosition == currentFoodPostion) {
    // Update total food ate
    totalFoodAte++;
    // Update in UI:
    document.getElementById("points").innerHTML = totalFoodAte;
